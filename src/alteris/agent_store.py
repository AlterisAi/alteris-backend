...
Each agent is a YAML file in ~/.alteris/agents/. The filename (without extension)
is the agent ID. Built-in example agents are shipped in src/alteris/agents/specs/
...
YAML was chosen over SQLite for agents because:
  ...
  - Transparent (ls ~/.alteris/agents/ shows everything)
...
from alteris.constants import AGENTS_DIR, BUILTIN_AGENTS_DIR
...
def ensure_agents_dir() -> Path:
    """Create ~/.alteris/agents/ if it doesn't exist. Seed with built-in specs."""
...
def list_agents(status: str | None = None) -> list[dict]:
    """List all agents from ~/.alteris/agents/. Optionally filter by status."""
    ensure_agents_dir()
    agents = []
