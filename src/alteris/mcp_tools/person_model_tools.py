...
Tools:
  alteris_estimate_person_model  — Run full estimation pipeline (scout → surveyor → save)
  alteris_get_person_model       — Get the latest person model
  alteris_person_model_start     — Sandwich phase 1: estimate + generate targeted questions
  alteris_person_model_chat      — Multi-turn onboarding chat to fill in gaps
  alteris_update_person_model    — Direct field update (bypass chat)
  alteris_person_model_finish    — Sync corrections to profile.yaml and return summary
"""
...
register_tool(ToolDef(
    name="alteris_estimate_person_model",
...
def _gather_user_kg_context(store: LayeredGraphStore) -> dict:
    ...
    # Profile.yaml
    from alteris.person_model import _load_user_config_safe
...
def generate_onboarding_questions(
    ...
    """
    from alteris.person_model import phase1_scout_all
    ...
    try:
        from alteris.constants import PERSON_MODEL_CONSIGLIERE_MODEL
...
def handle_person_model_start(
    ...
    **kwargs,
    ...
    from alteris.person_model import (
    ...
    static = [
        {
            ...
            "why": "Family structure is the single biggest factor in how Alteris interprets your calendar and priorities.",
            "current_evidence": "",
        },
        {
            ...
            "why": "Knowing your fixed blocks lets Alteris identify scheduling conflicts and protect your sacred time.",
            "current_evidence": "",
        },
        {
            ...
            "why": "Your local ecosystem helps Alteris make location-aware suggestions and understand your routines.",
            "current_evidence": "",
        },
        {
            ...
            "why": "Understanding your energy patterns helps Alteris time notifications and prioritize information delivery.",
            "current_evidence": "",
        },
        {
            ...
            "why": "Role context helps Alteris prioritize communications and understand meeting importance.",
            "current_evidence": "",
        },
    ]
...
register_tool(ToolDef(
    name="alteris_person_model_start",
...
def handle_get_person_model(
    ...
    **kwargs,
    ...
    from alteris.person_model import get_person_model, get_model_gaps

    existing = get_person_model(store)
    if not existing:
        return {"status": "empty", "message": "No person model exists. Run alteris_estimate_person_model first."}
...
You are the Alteris Person Model onboarding assistant. Your job is to map the user's \
...
def handle_person_model_chat(
    ...
    **kwargs,
    ...
    # Load start session context (from alteris_person_model_start)
    start_context = None
    if start_session_id:
    ...
    if existing:
        ...
        from alteris.person_model import _empty_model
        model = _empty_model()
    ...
    try:
        from alteris.llm.gemini import GeminiClient
        from alteris.constants import PERSON_MODEL_CHAT_MODEL
...
register_tool(ToolDef(
    name="alteris_person_model_chat",
    ...
    params=[
        ...
        ToolParam("start_session_id", "string", "Session ID from alteris_person_model_start (first message only)"),
...
register_tool(ToolDef(
    name="alteris_update_person_model",
...
You are the Alteris Person Model Compiler. Your sole function is to take a raw JSON object \
...
def handle_person_model_finish(
    ...
    **kwargs,
    ...
    from alteris.person_model import get_person_model
...
def _fallback_profile_yaml(model: dict, corrections: dict, existing_yaml: str) -> str:
    ...
    lines = ["# Alteris user profile", "# Updated by Person Model onboarding", ""]
...
register_tool(ToolDef(
    name="alteris_person_model_finish",
