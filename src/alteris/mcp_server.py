"""Alteris MCP Server — HTTP/SSE transport on localhost.
...
Usage:
    python -m alteris.mcp_server
    python -m alteris.mcp_server --port 9119
    python -m alteris.mcp_server --read-only  # Only expose read tools (for agents)
"""
...
from alteris.constants import MCP_DEFAULT_PORT, MCP_HOST
...
def create_server(
    ...
    server = Server("alteris-kg")

    # Load all tool definitions
    from alteris.mcp_tools import ensure_tools_loaded, get_all_tools, get_read_tools
    ensure_tools_loaded()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="alteris-mcp",
        description="Alteris MCP Server — Knowledge Graph interface",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Database path (default: ~/.alteris/graph.db)",
    )
    parser.add_argument(
