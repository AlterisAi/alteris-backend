...
def _extract_communication_frequency(
    ...
    Separates direct (1:1) vs group message counts so tier assignment
    can weight them differently — group-only contacts shouldn't rank
    as high as direct conversation partners.
    """
    rows = store.conn.execute(
        """SELECT pe.person_id, p.canonical_name,
                  COUNT(DISTINCT pe.event_id) as event_count,
                  SUM(CASE WHEN json_extract(e.metadata, '$.is_group') = 1
    ...
    for r in rows:
        ...
        direct_count = count - group_count
...
def _extract_communication_channels(
    ...
    Uses FULL event history (not time-windowed) because channels are a
    stable relationship characteristic — if someone emailed you 6 months
    ago and WhatsApp'd this week, both channels matter.
    """
    rows = store.conn.execute(
        """SELECT pe.person_id, p.canonical_name,
...
def extract_stage1_claims(
    ...
    if pe_count == 0:
        from alteris.linker import rebuild_person_events
...
def populate_person_profiles(store: LayeredGraphStore) -> dict[str, int]:
    ...
    for r in freq_rows:
        ...
        # Tier uses effective count: direct messages count full,
        # group messages count 5% — group-only contacts rank much lower
        direct_count = obj.get("direct_count", msg_count)
        group_count = obj.get("group_count", 0)
        effective_count = direct_count + int(group_count * 0.05)

        # Preliminary tier from effective message count
    ...
    # Multi-channel tier boost: presence across 3+ channels is a strong
    # signal of a real relationship — promote by one tier
    for pid, prof in profiles.items():
        channel_count = prof.get("channel_count", 0)
        if channel_count >= 3 and prof["tier"] > 1:
            prof["tier"] -= 1  # Promote one tier (e.g., tier 3 → tier 2)
