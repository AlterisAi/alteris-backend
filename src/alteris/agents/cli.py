...
Usage:
  python -m alteris.agents.cli cto "Find warm paths to Jose at Bunch Capital"
  python -m alteris.agents.cli ceo "Prep for tomorrow's meeting with Villi"
  python -m alteris.agents.cli research --name "Jose" --firm "Bunch Capital"
  python -m alteris.agents.cli pipeline
  python -m alteris.agents.cli drafts
  python -m alteris.agents.cli outreach --file vcs.json
"""
...
def cmd_agent(args: argparse.Namespace) -> None:
    ...
    if args.role == "cto":
        from alteris.agents.cto_agent import create_cto_agent
        agent = create_cto_agent(store, workspace)
    else:
        from alteris.agents.ceo_agent import create_ceo_agent
...
def cmd_research(args: argparse.Namespace) -> None:
    ...
    from alteris.agents.outreach import research_vc, qualify_and_route
    from alteris.agents.workspace import SharedWorkspace
...
def cmd_outreach(args: argparse.Namespace) -> None:
    ...
    from alteris.agents.outreach import run_outreach_batch
    from alteris.agents.workspace import SharedWorkspace
    from alteris.llm.gemini import GeminiClient
    from alteris.store import LayeredGraphStore
...
def cmd_drafts(args: argparse.Namespace) -> None:
    ...
    from alteris.agents.outreach import review_drafts, approve_draft, reject_draft
    from alteris.agents.workspace import SharedWorkspace
...
def cmd_approve(args: argparse.Namespace) -> None:
    ...
    from alteris.agents.outreach import approve_draft
    from alteris.agents.workspace import SharedWorkspace
...
def cmd_reject(args: argparse.Namespace) -> None:
    ...
    from alteris.agents.outreach import reject_draft
    from alteris.agents.workspace import SharedWorkspace
...
def cmd_sync(args: argparse.Namespace) -> None:
    ...
    from alteris.agents.discover import discover_vcs, add_discovered_to_pipeline
    from alteris.agents.recommend import recommend_actions
    from alteris.agents.workspace import SharedWorkspace
    from alteris.store import LayeredGraphStore
    ...
    if do_research:
        ...
        from alteris.agents.outreach import research_vc
        from alteris.agents.protocol import build_pitch_angle, score_vc_focus
        from alteris.llm.gemini import GeminiClient
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Alteris Agents — autonomous VC outreach with privacy-preserving coordination",
    )
    parser.add_argument(
        "--db", dest="db_path",
        default=str(Path.home() / ".alteris" / "graph.db"),
        help="Path to knowledge graph database",
    )
    parser.add_argument(
        "--workspace", default=str(Path.home() / ".alteris" / "workspace.db"),
