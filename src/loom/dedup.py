...
def verify_clusters_with_llm(
    ...
    Args:
        clusters: Layer 2 clusters to verify.
        llm_client: Gemini client instance.
        store: Graph store for looking up email domains per person.
        batch_size: Number of clusters per LLM call.
        save_path: If set, save raw responses to this path for recovery.

    Returns:
        List of LLM decisions, each with 'group', 'action', and optionally
        'canonical' or 'subgroups'.
    """
    ...
    for batch_start in range(0, len(clusters), batch_size):
        ...
        for retry in range(3):
            try:
                ...
                # Schema-enforced JSON — direct parse, no fallbacks needed
                ...
                logger.warning(
                    "LLM dedup batch %d retry %d: %s",
                    batch_start, retry, exc,
                )
                if retry == 2:
                    for i in range(len(batch)):
                        all_results.append({
                            "group": batch_start + i + 1,
                            "action": "reject",
                        })
    ...
    # Save raw responses for recovery
    if save_path and raw_responses:
        Path(save_path).parent.mkdir(parents=True, exist_ok=True)
        Path(save_path).write_text(json.dumps(raw_responses, indent=2))
        logger.info("Saved %d raw LLM responses to %s", len(raw_responses), save_path)
