...
def load_config() -> dict:
    """Load Loom config.json, searching multiple locations.

    Search order:
      1. ~/.loom/config.json  (canonical)
      2. ~/.alteris/config.json  (legacy)
      3. ~/Downloads/config.json  (for non-technical users)
      4. ~/Desktop/config.json
      5. ~/Documents/config.json

    If found outside ~/.loom/, the file is auto-copied to ~/.loom/config.json
...
"""User's local timezone for timing pattern extraction."""
...
def safe_timezone(tz_name: str | None = None):
    ...
    ``zoneinfo.ZoneInfo(key)`` to raise ``KeyError``. This helper
    catches that and returns UTC so the pipeline doesn't crash.
    """
    import zoneinfo
    from datetime import timezone

    key = tz_name or USER_TIMEZONE
    try:
        return zoneinfo.ZoneInfo(key)
    except (KeyError, zoneinfo.ZoneInfoNotFoundError):
        _logger.warning("Timezone %r not found (tzdata missing?), using UTC", key)
        return timezone.utc
...
SPEND_BYPASS_SALT = "loom_bypass_salt_2026"
