...
Profile is loaded from ~/.loom/profile.yaml (preferred) or
...
def load_profile() -> dict:
    """Load profile.yaml from ~/.loom or ~/.alteris, JSON fallback.

    Returns the raw dict from YAML parsing. Handles both the v2
    hierarchical format (~/.loom) and v1 flat format (~/.alteris).
    Returns empty dict on failure.
    """
    from loom.constants import LOOM_DIR

    for yaml_path in [
        LOOM_DIR / "profile.yaml",
    ...
    for path in [LOOM_DIR / "config.json", Path.home() / ".alteris" / "config.json"]:
        if path.exists():
            try:
...
def format_profile_context(profile: dict | None) -> str:
    ...
    Handles both hierarchical (.loom format) and flat (.alteris format).
