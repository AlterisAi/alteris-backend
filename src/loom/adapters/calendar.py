...
def _load_user_emails() -> set[str]:
    ...
    db_path = os.path.expanduser("~/.loom/graph.db")
    ...
    if not emails:
        for path in ("~/.loom/config.json", "~/.alteris/config.json"):
            try:
...
class CalendarAdapter(SourceAdapter):
    ...
    def ingest(self, since_ts: int = 0, limit: int = 0) -> IngestResult:
        ...
        logger.info("Calendar EventKit authorization status: %d", status)
            ...
            if not grant_result[0]:
                ...
                return IngestResult(
                    ...
                    errors=[
                        ...
                        "then enable access for Loom (or loom-cli/Python). "
                        "If you don't see it listed, try running the pipeline "
                        "once from Terminal: python -m loom.cli ingest"
                    ],
                )
        elif status == 2:
