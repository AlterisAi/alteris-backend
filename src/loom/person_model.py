...
Usage:
    from loom.person_model import estimate_person_model, get_person_model
...
def _scout_identity(conn, now: int) -> dict:
    ...
    if not result["timezone"]:
        from loom.constants import USER_TIMEZONE
...
def _scout_financial_landscape(conn, now: int) -> dict:
    ...
    # Transaction emails (classified by Apple Intelligence's on-device model, not Loom)
...
def _scout_apple_mail_categories(conn, now: int) -> dict:
    ...
    running in Mail.app — NOT by Loom. The model_category field is written by macOS
...
def _scout_apple_intelligence_signals(conn, now: int) -> dict:
    ...
    These signals are produced by Apple's on-device ML models, NOT by Loom:
    ...
    All classification happens on-device in macOS — Loom just reads the results.
    """
    result: dict = {
...
## Apple Intelligence Signals (classified by Apple, NOT by Loom)
The scout data includes two sections produced by Apple's on-device ML models
(Apple Intelligence), not by Loom's own analysis:
