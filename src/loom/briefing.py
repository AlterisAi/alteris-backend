...
from loom.constants import CLOUD_FAST_MODEL, SECONDS_PER_DAY, USER_TIMEZONE, safe_timezone
from loom.llm.base import LLMClient
from loom.prompts.briefing import (
...
def _get_cross_source_links(
    ...
    """
    from loom.cross_source import find_related_events
...
def _build_briefing_prompt(
    ...
    if not orphaned:
        ...
        # Group: overdue, due soon, no deadline, future
        overdue = [c for c in orphaned if c.is_overdue]
        due_soon = [
            c for c in orphaned
        ...
        future = [
            c for c in orphaned
            if c.deadline and not c.is_overdue
            and (c.deadline_in_days is None or c.deadline_in_days > 7)
        ]

        for label, group in [
            ("OVERDUE", overdue),
            ("DUE WITHIN 7 DAYS", due_soon),
            ("NO DEADLINE (recent promises)", no_deadline),
            ("FUTURE DEADLINES", future),
        ]:
            if group:
...
def run_briefing(
    ...
    if not skip_anticipation and events:
