...
from loom.prompts.beliefs import (
...
def _build_triage_context(bundle: ThreadBundle) -> str:
    """Build triage summary section from bundle's triage_data."""
...
def synthesize_thread(
    ...
    Single Flash 3 call per thread. Returns (commitment_claims, msg_id_map).
    ...
    use_thinking = "lite" not in (model or "").lower()
    ...
    raw_str = llm_client.generate(
        ...
        model=model,
        temperature=0.1,
        thinking_level="low" if use_thinking else None,
...
def _build_participant_inference_beliefs(
    ...
    from loom.linker import infer_meeting_participants_from_calls
...
def run_synthesis(
    ...
    # Flash Lite doesn't support thinking
    batch_use_thinking = "lite" not in (model or "").lower()

    def _synth_from_prompt(
        item: tuple[ThreadBundle, str, str, dict[str, str], list[dict]],
    ) -> tuple[ThreadBundle, list[Claim], dict[str, str], list[dict]]:
        ...
        raw_str = llm_client.generate(
            ...
            model=model,
            temperature=0.1,
            thinking_level="low" if batch_use_thinking else None,
        ...
        return bundle, claims, msg_id_map, prior
    ...
    if workers <= 1:
        for item in prebuilt:
            try:
                bundle, claims, _, prior = _synth_from_prompt(item)
                _write_synthesis_results(bundle, claims, prior)
                synth_done += 1
                if synth_done % 50 == 0:
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(futures):
                item = futures[future]
                try:
                    bundle, claims, _, prior = future.result()
                    _write_synthesis_results(bundle, claims, prior)
                except Exception as e:
                    logger.error(
                        "Synthesis failed for %s: %s",
