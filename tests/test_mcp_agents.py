...
def tmp_agents_dir(tmp_path):
    ...
    agents_dir.mkdir()
    with patch("loom.agent_store.AGENTS_DIR", agents_dir), \
         patch("loom.agent_store.BUILTIN_AGENTS_DIR", tmp_path / "builtins"):
        yield agents_dir
...
def tmp_agents_with_builtins(tmp_path):
    ...
    spec = {
        ...
        "tool_permissions": ["loom_search", "loom_query_persons"],
        "status": "active",
    ...
    with patch("loom.agent_store.AGENTS_DIR", agents_dir), \
         patch("loom.agent_store.BUILTIN_AGENTS_DIR", builtins_dir):
...
class TestAgentStore:
    ...
    def test_create_agent(self, tmp_agents_dir):
        from loom.agent_store import create_agent, get_agent

        agent_id = create_agent(
            name="Test Agent",
            system_prompt="You are a test agent.",
            tool_permissions=["loom_search"],
        ...
        assert agent["tool_permissions"] == ["loom_search"]
    ...
    def test_create_agent_dedup_id(self, tmp_agents_dir):
        from loom.agent_store import create_agent
    ...
    def test_list_agents_empty(self, tmp_agents_dir):
        from loom.agent_store import list_agents
        assert list_agents() == []

    def test_list_agents(self, tmp_agents_dir):
        from loom.agent_store import create_agent, list_agents
    ...
    def test_update_agent(self, tmp_agents_dir):
        from loom.agent_store import create_agent, get_agent, update_agent
    ...
    def test_update_nonexistent(self, tmp_agents_dir):
        from loom.agent_store import update_agent
        assert update_agent("nonexistent", name="X") is False

    def test_delete_agent(self, tmp_agents_dir):
        from loom.agent_store import create_agent, delete_agent, get_agent
    ...
    def test_builtin_seeding(self, tmp_agents_with_builtins):
        from loom.agent_store import ensure_agents_dir, list_agents
    ...
    def test_builtin_not_overwritten(self, tmp_agents_with_builtins):
        from loom.agent_store import ensure_agents_dir
    ...
    def test_tool_permissions_roundtrip(self, tmp_agents_dir):
        ...
        perms = ["loom_search", "loom_query_beliefs", "loom_person_detail"]
...
class TestMCPToolRegistration:
    ...
    def test_all_tools_registered(self):
        ...
        # KG read tools
        assert "loom_stats" in names
        assert "loom_query_events" in names
        assert "loom_query_beliefs" in names
        assert "loom_query_persons" in names
        assert "loom_query_commitments" in names
        assert "loom_person_detail" in names
    ...
    def test_read_tools_safe_for_agents(self):
        from loom.mcp_tools import ensure_tools_loaded, get_read_tools
        ensure_tools_loaded()
        read_tools = get_read_tools()
        names = {t.name for t in read_tools}

        # These should be read-only
        for name in ["loom_stats", "loom_query_events", "loom_query_beliefs",
                      "loom_query_persons", "loom_query_commitments",
                      "loom_person_detail", "loom_search", "agent_list"]:
...
class TestAgentMCPTools:
    ...
    def test_agent_create_via_tool(self, tmp_agents_dir, store):
        from loom.mcp_tools.agent_tools import handle_agent_create
    ...
    def test_agent_create_invalid_perms(self, tmp_agents_dir, store):
        ...
        result = handle_agent_create(
            ...
            name="Bad",
            system_prompt="Hi",
            tool_permissions=["loom_run_pipeline"],
        )
        assert "error" in result
        assert "Invalid" in result["error"]

    def test_agent_list_via_tool(self, tmp_agents_dir, store):
        from loom.mcp_tools.agent_tools import handle_agent_create, handle_agent_list
    ...
    def test_agent_update_via_tool(self, tmp_agents_dir, store):
        from loom.mcp_tools.agent_tools import handle_agent_create, handle_agent_update
    ...
    def test_agent_delete_via_tool(self, tmp_agents_dir, store):
        from loom.mcp_tools.agent_tools import handle_agent_create, handle_agent_delete, handle_agent_list
...
class TestCQAutoBucketing:
    ...
    def test_no_deadline_goes_to_background(self):
        from loom.mcp_tools.cq_tools import _auto_bucket
...
class TestCQGetState:
    ...
    def test_empty_state(self, store):
        from loom.mcp_tools.cq_tools import handle_cq_get_state
    ...
    def test_manual_task_appears(self, store):
        from loom.mcp_tools.cq_tools import handle_cq_add_task, handle_cq_get_state
    ...
    def test_dismiss_removes_from_state(self, store):
        from loom.mcp_tools.cq_tools import handle_cq_add_task, handle_cq_dismiss, handle_cq_get_state
...
class TestCQTaskCRUD:
    ...
    def test_add_task(self, store):
        from loom.mcp_tools.cq_tools import handle_cq_add_task
    ...
    def test_update_task(self, store):
        from loom.mcp_tools.cq_tools import handle_cq_add_task, handle_cq_update_task
    ...
    def test_move_task(self, store):
        from loom.mcp_tools.cq_tools import handle_cq_add_task, handle_cq_get_state, handle_cq_move_task
...
class TestAgentRuntime:
    ...
    def test_build_scoped_tools(self, store):
        from loom.agent_runtime import _build_scoped_tools

        tools = _build_scoped_tools(store, ["loom_stats", "loom_search"])
        assert "loom_stats" in tools
        assert "loom_search" in tools
        assert "loom_run_pipeline" not in tools
    ...
    def test_scoped_tools_invalid_name_skipped(self, store):
        ...
        tools = _build_scoped_tools(store, ["loom_stats", "nonexistent_tool"])
        assert "loom_stats" in tools
...
class TestKGTools:
    ...
    def test_loom_stats(self, store):
        from loom.mcp_tools.kg_tools import handle_loom_stats

        result = handle_loom_stats(store)
        assert "events_count" in result
        assert "claims_count" in result
        assert "beliefs_count" in result
        assert "persons_count" in result

    def test_loom_query_events_empty(self, store):
        from loom.mcp_tools.kg_tools import handle_loom_query_events

        result = handle_loom_query_events(store)
        assert "events" in result
        assert result["count"] == 0

    def test_loom_query_beliefs_empty(self, store):
        from loom.mcp_tools.kg_tools import handle_loom_query_beliefs

        result = handle_loom_query_beliefs(store)
        assert "beliefs" in result
        assert result["count"] == 0

    def test_loom_query_persons_empty(self, store):
        from loom.mcp_tools.kg_tools import handle_loom_query_persons

        result = handle_loom_query_persons(store)
        assert "persons" in result
        assert result["count"] == 0

    def test_loom_search_empty(self, store):
        from loom.mcp_tools.kg_tools import handle_loom_search

        result = handle_loom_search(store, query="test")
        assert "events" in result
        assert result["count"] == 0
...
class TestVCResearchAgent:
    ...
    def test_spec_file_exists(self):
        from loom.constants import BUILTIN_AGENTS_DIR
    ...
    def test_spec_tool_permissions_valid(self):
        from loom.constants import AGENT_READ_TOOLS, BUILTIN_AGENTS_DIR
        spec_path = BUILTIN_AGENTS_DIR / "vc-research.yaml"
        with open(spec_path) as f:
            data = yaml.safe_load(f)
    ...
    def test_spec_seeded_on_ensure(self, tmp_agents_with_builtins):
        ...
        from loom.agent_store import ensure_agents_dir, get_agent
